...
class PrintController extends Controller
{
    public function all()
    {
        ...
        $query = SupplierPrint::where('status', $status)->orderBy($orderBy, $orderByType);
        ...
        foreach ($data->items() as $item) {
            $item->opening = $item->logs()->where('supplier_id',$item->id)->where('supplier_type','print')->where('payment_type', 'opening')->sum('amount');
            $item->bill = $item->logs()->where('supplier_id',$item->id)->where('supplier_type','print')->where('payment_type', 'bill')->sum('amount');
            $item->payment = $item->logs()->where('supplier_id',$item->id)->where('supplier_type','print')->where('payment_type', 'payment')->sum('amount');
    ...
    {
        $data = SupplierPrint::where('id',$id)->first();
        if(!$data){
    ...
    public function store(Request $request)
    {
        ...
        $data = SupplierPrint::create(request()->except('mobile_number'));
    ...
    {
        $data = SupplierPrint::find(request()->id);
        if(!$data){
            return response()->json([
                'err_message' => 'validation error',
    ...
    public function bulk_import()
    {
        ...
        foreach (request()->data as $item) {
            ...
            $item = (object) $item;
            $check = SupplierPrint::where('id',$item->id)->first();
            if(!$check){
                try {
                    SupplierPrint::create((array) $item);
                } catch (\Throwable $th) {
                    return response()->json([
                        'err_message' => 'validation error',
