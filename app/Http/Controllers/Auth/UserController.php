...
class UserController extends Controller
{
    ...
    {
        $role = UserRole::where('role_serial', 4)->first();
        ...
        $query = $role->users()->where('status', $status)->orderBy($orderBy, $orderByType);
        ...
        foreach ($users->items() as $user) {
            $total_order = $user->orders()->where('sales_id', '>', 0)->sum('total_price');
            $total_paid = $user->orders()->where('sales_id', '>', 0)->sum('total_paid');
            $user->advanced = $user->payment_remains->amount ?? 0;

            $user->total_order = $total_order;
            $user->total_paid = $total_paid;
            $user->due = $total_order - $total_paid;
        }
        return response()->json($users);
    }
