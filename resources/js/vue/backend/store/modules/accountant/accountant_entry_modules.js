...
const actions = {
    ...
    /** branch payment store */
    [`store_branch_payment`]: function ({ state, getters, commit, rootState }) {

        const { form_values, form_inputs, form_data } = window.get_form_data('.create_form');
        // const { get_accountant_category_type_selected: types } = getters;
        // types.forEach(i => form_data.append('type_id', i.id));

        const { get_user_selected: user } = getters;
        if(user.length){
            form_data.append('name', user[0]?.first_name);
            form_data.append('user_id', user[0]?.id);
        }

        axios.post(`/accountant/branch-payment/store`, form_data)
            .then(res => {
                $('.create_form').trigger('reset');
