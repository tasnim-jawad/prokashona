<template>
    <div class="container-fluid">
        <div class="card list_card">
            ...
            <form @submit.prevent="call_store(`update_production_production_order`,{target: $event.target, categories: categories})" autocomplete="false">
                <div class="card-body">
                                ...
                                <div class=" form-group full_width d-grid align-content-start gap-1 mb-2 " >
                                    ...
                                    <div class="production_supplier_selection_table_body">
                                        ...
                                        <div v-for="(i, index) in suppliers" :key="i" class="production_supplier_selection_table">
                                            ...
                                            <div class="action">
                                                <a href="" v-if="suppliers.length > 1" @click.prevent="delete_suppliers(index)" class="btn btn-sm btn-outline-danger">
...
export default {
    ...
    created: async function () {
        ...
        that.data.related_suppliers.forEach(function(category) {
            // console.log(category);
            let cat = that.categories.find(i=>i.id == category.category_id);
            if(cat){
                cat.supplier_id = category.supplier_id;
                cat.selected_name = category.supplier_name;
                cat.amount = category.amount;
                cat.price = category.price;
                cat.unit = category.unit;
                cat.cost = category.cost;
                cat.order_number = category.order_number;
                cat.comment = category.comment;
            }
        });
        // that.suppliers = that.data.paper_suppliers.map((i,index)=>index+1);
        that.data.paper_suppliers.forEach((i,index)=>{
            let amount_el = document.querySelector(`[name="paper_supplier[${index}][amount]"]`);
            if(amount_el){
                amount_el.value = i.amount;
            }
        });
        that.printing_qty = that.data.print_qty;
        that.each_copy_price = that.data.each_copy_price;
